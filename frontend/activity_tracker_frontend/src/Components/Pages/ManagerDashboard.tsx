...
const ManagerDashboard: React.FC = () => {
  ...
  const tasksByEmployee: Record<string, { name: string, total: number, completed: number, inProgress: number, todo: number }> = {};
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-blue-900 py-10">
        <div className="max-w-7xl mx-auto p-8 bg-gray-800 rounded-xl shadow-2xl">
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredTasks.map((task) => (
                    <tr key={task._id} className="border-b border-gray-700 hover:bg-gray-800">
                      <td className="px-4 py-4 text-gray-300">{task.assignedTo?.name || "Unknown"}</td>
                      <td className="px-4 py-4 text-gray-300">{task.title}</td>
                      <td className="px-4 py-4 text-gray-300">{task.estimatedTime}</td>
                      ...
                      <td className="px-4 py-4 text-gray-300">{task.date ? formatDate(task.date) : "N/A"}</td>                      <td className="px-4 py-4">
                        <button
          ...
          <div className="bg-gray-900 rounded-xl p-6 shadow-lg">
            <h2 className="text-2xl font-bold text-white mb-4">Employee Performance</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.keys(tasksByEmployee).map(employeeId => {
                const employee = tasksByEmployee[employeeId];
                const completionRate = employee.total > 0 
                  ? Math.round((employee.completed / employee.total) * 100) 
                  : 0;
                
                return (
                  <div key={employeeId} className="bg-gray-800 rounded-lg p-4 shadow">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center text-sm">
                      <div>
                        <div className="text-green-400 font-medium">{employee.completed}</div>
                        <div className="text-gray-500">Completed</div>
                      </div>
                      <div>
                        <div className="text-yellow-400 font-medium">{employee.inProgress}</div>
                        <div className="text-gray-500">In Progress</div>
                      </div>
                      <div>
                        <div className="text-gray-400 font-medium">{employee.todo}</div>
